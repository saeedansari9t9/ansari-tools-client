...
const CanvaSubscriptionsPage = () => {
  ...
  const fetchSubscriptions = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/canva-subscriptions?limit=1000`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/canva-subscriptions/${id}`, {
        method: 'DELETE'
      });
